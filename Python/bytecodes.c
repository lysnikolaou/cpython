...
// BEGIN BYTECODES //
        ...
        inst(BUILD_INTERPOLATION, (values[oparg] -- interpolation)) {
            ...
            PyObject *interpolation_obj = _PyInterpolation_FromStackRefSteal(values, oparg);
            ERROR_IF(interpolation_obj == NULL, error);
            interpolation = PyStackRef_FromPyObjectSteal(interpolation_obj);
        }

        inst(BUILD_DECODED, (s -- decoded)) {
            PyObject *decoded_obj = _PyDecodedConcrete_FromStackRefSteal(s);
            ERROR_IF(decoded_obj == NULL, error);
            decoded = PyStackRef_FromPyObjectSteal(decoded_obj);
        }
