...
add_ast_annotations(struct ast_state *state)
{
    ...
    Py_DECREF(JoinedStr_annotations);
    PyObject *TagString_annotations = PyDict_New();
    if (!TagString_annotations) return 0;
    {
        PyObject *type = state->expr_type;
        Py_INCREF(type);
        cond = PyDict_SetItemString(TagString_annotations, "tag", type) == 0;
        Py_DECREF(type);
        if (!cond) {
    ...
    }
    {
        PyObject *type = state->expr_type;
        Py_INCREF(type);
        cond = PyDict_SetItemString(TagString_annotations, "str", type) == 0;
        Py_DECREF(type);
        if (!cond) {
            Py_DECREF(TagString_annotations);
            return 0;
    ...
    cond = PyObject_SetAttrString(state->TagString_type, "_field_types",
                                  TagString_annotations) == 0;
    if (!cond) {
        Py_DECREF(TagString_annotations);
        return 0;
    }
    cond = PyObject_SetAttrString(state->TagString_type, "__annotations__",
                                  TagString_annotations) == 0;
    if (!cond) {
        Py_DECREF(TagString_annotations);
        return 0;
...
ast2obj_expr(struct ast_state *state, struct validator *vstate, void* _o)
{
    ...
    case TagString_kind:
        ...
        result = PyType_GenericNew(tp, NULL, NULL);
        if (!result) goto failed;
        value = ast2obj_expr(state, vstate, o->v.TagString.tag);
        if (!value) goto failed;
        if (PyObject_SetAttr(result, state->tag, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_expr(state, vstate, o->v.TagString.str);
        if (!value) goto failed;
