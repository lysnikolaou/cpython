...
_PyLexer_remember_fstring_buffers(struct tok_state *tok)
{
    ...
    for (index = tok->tok_mode_stack_index; index >= 0; --index) {
        ...
        mode->f_string_start_offset = mode->string_start - tok->buf;
        mode->f_string_multi_line_start_offset = mode->string_multi_line_start - tok->buf;
    }
}
...
_PyLexer_restore_fstring_buffers(struct tok_state *tok)
{
    ...
    for (index = tok->tok_mode_stack_index; index >= 0; --index) {
        mode = &(tok->tok_mode_stack[index]);
        mode->string_start = tok->buf + mode->f_string_start_offset;
        mode->string_multi_line_start = tok->buf + mode->f_string_multi_line_start_offset;
    }
}
